...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="dtb:noteref|dtb:annoref">
    ...
    <!-- If a pagenum immediately follows, omit the closing separator line — the
         pagenum's own leading newline closes it, avoiding a blank line before
         the page marker (http://redmine.sbszh.ch/issues/3015). -->
    <xsl:text>&#10;* </xsl:text>
	...
	<xsl:variable name="map">
	  <xsl:choose>
        ...
        <xsl:choose>
          <xsl:when test="not($isFirst) or not($isLast) or (count(tokenize(string(.), '(\s|&#xA0;|/|-)+')[string(.) ne '']) > 1)">
            ...
            <xsl:if test="$isFirst">
              ...
              <xsl:value-of select="louis:translate(string($braille_tables), string($map/entry[@key='start-multi']))"/>
            </xsl:if>
            <xsl:apply-templates/>
            <xsl:if test="$isLast">
              <!-- Announce the end of emphasis -->
              <xsl:value-of select="louis:translate(string($braille_tables), string($map/entry[@key='end-multi']))"/>
            </xsl:if>
          </xsl:when>
          <xsl:otherwise>
            ...
            <xsl:choose>
              ...
              <xsl:when
                ...
                <xsl:value-of select="louis:translate(string($braille_tables), string($map/entry[@key='start-single']))"/>
		...
		<xsl:value-of select="louis:translate(string($braille_tables), concat(string(),'&#x250A;'))"/>
                <xsl:value-of select="louis:translate(string($braille_tables), string($map/entry[@key='end-single']))"/>
              </xsl:when>
              ...
              <xsl:when
                ...
                <xsl:value-of select="louis:translate(string($braille_tables), string($map/entry[@key='start-inside']))"/>
  ...
  <xsl:template match="brl:time">
    ...
    <xsl:sequence select="louis:translate(string($braille_tables), translate(string(.),':', '&#x2573;'))"/>
  </xsl:template>
  ...
  <xsl:template
    match="text()[(matches(string(), '^ich$', 'i') or matches(string(), '\Wich$', 'i')) and matches(string(my:following-textnode-within-block(.)), '^[,;:?!)&#x00bb;&#x00ab;]')]" priority="100">
    <xsl:variable name="braille_tables">
      <xsl:call-template name="getTable"/>
    </xsl:variable>
