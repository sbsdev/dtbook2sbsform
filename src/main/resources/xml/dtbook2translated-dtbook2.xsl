    ...
    <xsl:if test="$context != 'date_month' and $context != 'date_day'">
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <xsl:choose>
            <xsl:when test="$accented_characters = 'de-accents-ch'">
              <xsl:text>sbs-de-accents-ch.mod,</xsl:text>
            </xsl:when>
            <xsl:when test="$accented_characters = 'de-accents-reduced'">
              <xsl:text>sbs-de-accents-reduced.mod,</xsl:text>
            </xsl:when>
            <xsl:otherwise>
              <xsl:text>sbs-de-accents.mod,</xsl:text>
    ...
    <xsl:for-each select="tokenize(string(@value), '-')">
      ...
      <xsl:choose>
        <xsl:when test="position() = 1">
          <xsl:value-of
            select="my:louis-translate($day_braille_tables, format-number(. cast as xs:integer,'#'))"
          />
        </xsl:when>
        <xsl:when test="position() = 2">
          <xsl:value-of
            select="my:louis-translate($month_braille_tables, format-number(. cast as xs:integer,'#'))"
          />
        </xsl:when>
        <xsl:otherwise>
	  <xsl:if test="matches(string(.), '\d+')">
	    <xsl:value-of
		select="my:louis-translate($braille_tables, format-number(. cast as xs:integer,'#'))"/>
	  </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:for-each>
